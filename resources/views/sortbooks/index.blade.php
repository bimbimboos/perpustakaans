...
@section('content')
    ...
    <!-- Form pencarian -->
    <form action="{{ route('sortbooks.index') }}" method="GET" class="mb-3">
        <div class="input-group" style="max-width:400px">
            ...
            @if(request('search'))
                <a href="{{ route('sortbooks.index') }}" class="btn btn-dark">Reset</a>
            @endif
        </div>
    </form>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped">
            ...
            @forelse($sortbooks as $sort)
                <tr class="text-center">
                    <!-- Nomor urut berdasarkan pagination -->
                    <td>{{ ($sortbooks->currentPage() - 1) * $sortbooks->perPage() + $loop->iteration }}</td>
                    <!-- Nama buku dari relasi -->
                    <td>{{ $sort->books->judul ?? 'Buku tidak ditemukan' }}</td>
                    <!-- Nama rak dari relasi -->
                    <td>{{ $sort->racks->nama ?? 'Rak tidak ditemukan' }}</td>
                    <td>{{ $sort->jumlah }}</td>
                    <td>{{ $sort->user->name }}</td>  <!-- Langsung dari field name (string) -->
                    <td>
                        <!-- Tombol lihat -->
                        <button type="button" class="btn btn-info btn-sm"
                                data-bs-toggle="modal"
                                data-bs-target="#modalShowPenataan{{ $sort->id_penataan }}">
                            Lihat
                        </button>
                        ...
                        @unless(Auth::user()->role === 'konsumen')
                            <button class="btn btn-sm btn-warning"
                                    data-bs-toggle="modal"
                                    data-bs-target="#modalEditPenataan{{ $sort->id_penataan }}">Edit</button>
                            <button type="button" class="btn btn-sm btn-danger"
                                    data-bs-toggle="modal"
                                    data-bs-target="#modalHapus{{ $sort->id_penataan }}">Hapus</button>
                        @endunless
                    </td>
                </tr>

                <!-- Modal Lihat -->
                <div class="modal fade" id="modalShowPenataan{{ $sort->id_penataan }}" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered modal-md">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <p><strong>Nama Buku:</strong> {{ $sort->books->judul ?? '-' }}</p>
                                <p><strong>Nama Rak:</strong> {{ $sort->racks->nama ?? '-' }}</p>
                                <p><strong>Kolom:</strong> {{ $sort->kolom }}</p>
                                <p><strong>Baris:</strong> {{ $sort->baris }}</p>
                                <p><strong>Jumlah:</strong> {{ $sort->jumlah }}</p>
                                <p><strong>Petugas:</strong> {{ $sort->user->name }}</p>  <!-- Langsung dari field name -->
                                <p><strong>Sumber:</strong>{{ $sort->sumber }}</p>
                            ...
                            <div class="modal-footer">
                                {{-- tombol baru --}}
                                @if($sort->racks)
                                    <a href="{{ url('/racks/' . $sort->racks->id_rak . '?kategori=' . $sort->books->id_kategori) }}" class="btn btn-primary">
                                        Lihat Rak
                                    </a>
                                @endif
    ...
    <div class="d-flex justify-content-center">
        {{ $sortbooks->links('pagination::bootstrap-5') }}
    </div>

    <!-- Modal Edit Penataan -->
    @foreach($sortbooks as $sort)
        <div class="modal fade" id="modalEditPenataan{{ $sort->id_penataan }}" tabindex="-1 "
             aria-labelledby="modalEditPenataanLabel{{ $sort->id_penataan }}" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content shadow-lg">
                    <div class="modal-header bg-warning text-white">
                        <h5 class="modal-title" id="modalEditPenataanLabel{{ $sort->id_penataan }}">Edit Penataan Buku</h5>
                    ...
                    <form action="{{ route('sortbooks.update', $sort->id_penataan) }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="selected_judul_buku_edit_{{ $sort->id_penataan }}">Buku</label>
                                <div class="input-group">
                                    <input type="text" id="selected_judul_buku_edit_{{ $sort->id_penataan }}" class="form-control" readonly
                                           placeholder="-- Pilih Buku --" value="{{ old('judul', $sort->books->judul ?? '') }}">
                                    <input type="hidden" name="id_buku" id="selected_id_buku_edit_{{ $sort->id_penataan }}"
                                           value="{{ old('id_buku', $sort->id_buku) }}">
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalPilihBuku"
                                            data-form-type="edit" data-penataan-id="{{ $sort->id_penataan }}">
                                        Pilih Buku
                                    </button>
                                </div>
                                @error('id_buku')
                            ...
                            <!-- Dropdown rak (tetap) -->
                            <div class="mb-3">
                                <label for="id_rak_{{ $sort->id_penataan }}">Rak</label>
                                <select name="id_rak" id="id_rak_{{ $sort->id_penataan }}" class="form-control" required>
                                    <option value="">-- Pilih Rak --</option>
                                    @foreach($rack as $r)
                                        <option value="{{ $r->id_rak }}"
                                            {{ old('id_rak', $sort->id_rak) == $r->id_rak ? 'selected' : '' }}>
                                            {{ $r->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                <label for="name_{{ $sort->id_penataan }}">Petugas</label>
                                <input type="text" class="form-control" id="name_{{ $sort->id_penataan }}" value="{{ Auth::user()->name }}" readonly>
                                <input type="hidden" name="name" value="{{ Auth::user()->name }}">  <!-- Kirim auto ke controller -->
                            </div>
                            <!-- Input kolom -->
                            <div class="mb-3">
                                <label for="kolom_{{ $sort->id_penataan }}">Kolom</label>
                                <input type="number" name="kolom" id="kolom_{{ $sort->id_penataan }}"
                                       class="form-control" value="{{ old('kolom', $sort->kolom) }}" min="1" required>
                                @error('kolom')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <!-- Input baris -->
                            <div class="mb-3">
                                <label for="baris_{{ $sort->id_penataan }}">Baris</label>
                                <input type="number" name="baris" id="baris_{{ $sort->id_penataan }}"
                                       class="form-control" value="{{ old('baris', $sort->baris) }}" min="1" required>
                                @error('baris')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <!-- Input jumlah -->
                            <div class="mb-3">
                                <label for="jumlah_{{ $sort->id_penataan }}">Jumlah</label>
                                <input type="number" name="jumlah" id="jumlah_{{ $sort->id_penataan }}"
                                       class="form-control" value="{{ old('jumlah', $sort->jumlah) }}" min="1" required>
                                @error('jumlah')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                <label for="sumber_{{ $sort->id_penataan }}">Sumber</label>
                                <input type="text" name="sumber" id="sumber_{{ $sort->id_penataan }}"
                                       class="form-control" value="{{ old('sumber', $sort->sumber) }}" required>
                                @error('sumber')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
    ...
    @foreach($sortbooks as $sort)
        <div class="modal fade" id="modalHapus{{ $sort->id_penataan }}" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Yakin ingin menghapus Penataan <b>{{ $sort->id_penataan }}</b>?
                    </div>
                    <div class="modal-footer">
                        <form action="{{ route('sortbooks.destroy', $sort->id_penataan) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadRak(idBuku, rackSelectId) {
            ...
            modalPilihBuku.addEventListener('show.bs.modal', function(event) {
                ...
                currentSortId = button.getAttribute('data-penataan-id') || '';
            ...
            modalPilihBuku.addEventListener('hidden.bs.modal', function() {
                ...
                } else if (currentFormType === 'edit' && currentSortId) {
                    const modalEdit = document.querySelector(`#modalEditPenataan${currentSortId}`);
            ...
            function loadBukuTable(search = '', page = 1) {
                const url = `/get-books-for-selection?search=${encodeURIComponent(search)}&page=${page}`;
                fetch(url)
                    .then(res => res.text())
                    .then(html => {
                ...
                pilihButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        if (currentFormType === 'tambah') {
                            ...
                            document.getElementById(`selected_id_buku_edit_${currentSortId}`).value = idBuku;
                            document.getElementById(`selected_judul_buku_edit_${currentSortId}`).value = judulBuku;
                            loadRak(idBuku, `id_rak_${currentSortId}`);
                        }
