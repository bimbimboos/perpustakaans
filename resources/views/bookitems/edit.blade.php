...
@section('content')
    <h2 class="mb-3">Edit Item Buku: {{ $book->judul }}</h2>
    ...
    <form action="{{ route('books.items.update', [$book->id_buku, $item->id_item]) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="rack" id="rack" value="{{ old('rack', $item->id_rak) }}" class="form-control">
        </div>

        <button type="submit" class="btn btn-primary">Simpan Perubahan</button>
        <a href="{{ route('books.items.index', $book->id_buku,$item->id_item) }}" class="btn btn-secondary">Batal</a>
    </form>
@endsection
