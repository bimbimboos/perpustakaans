...
@section('content')
    <h2 class="mb-3">Detail Item Buku: {{ $book->judul }}</h2>
    ...
    <div class="mt-3">
        <a href="{{ route('books.items.index', $book->id_buku) }}" class="btn btn-secondary">← Kembali</a>
        <a href="{{ route('books.items.edit', [$book->id_buku, $item->id_item]) }}" class="btn btn-warning">Edit</a>
        <form action="{{ route('books.items.destroy', [$book->id_buku, $item->id_item]) }}" method="POST" class="d-inline">
            @csrf
            @method('DELETE')
